...
**智能AI面试平台** - 基于大语言模型的简历分析、模拟面试和 RAG 知识库系统
...
> **二开新增**：用户登录认证系统（JWT + BCrypt）、国际化 i18n 中英文切换、图形验证码登录保护、护眼深绿色主题、GitHub Actions CI/CD 流水线。
...
| JWT                  | 0.12.6 | JWT 令牌生成与验证（二开新增）|
| Spring Security Crypto| 6.4.2 | BCrypt 密码加密（二开新增，仅加密模块）|
...
| i18next           | 26.3  | 国际化框架（二开新增）|
| react-i18next     | 17.0  | React 国际化集成（二开新增）|
| pnpm              | 10.26 | 前端包管理器   |
...
- **用户注册登录**：基于 JWT 的无状态认证，支持用户名密码注册与登录
- **密码安全加密**：使用 BCrypt 算法加密存储用户密码
- **API 认证拦截**：`AuthInterceptor` + `@RequireAuth` 注解实现按需鉴权，前端 Axios 拦截器自动携带 Token
- **图形验证码**：登录页增加验证码校验，有效防止暴力破解与机器人登录
- **国际化 i18n**：完整的中文/英文语言包，浏览器语言自动检测 + localStorage 持久化
- **语言选择器**：Layout 底部导航集成语言切换按钮，一键切换中英文
- **护眼主题色**：全站蓝紫色系改为 `#6B8F71` 护眼深绿色，降低长时间使用的视觉疲劳
...
本项目在原版基础上，二次开发增加了以下功能：

### 用户登录认证系统
- **JWT 令牌认证**：集成 JWT（JSON Web Token）实现无状态登录，支持 Token 自动过期刷新
- **注册/登录接口**：RESTful 认证接口，支持用户名密码注册与登录
- **密码安全加密**：使用 BCrypt 算法加密存储用户密码
- **请求拦截器**：前端 Axios 拦截器自动携带 Token，后端 AuthInterceptor 统一鉴权
- **全局认证守卫**：基于 `AuthInterceptor` + `@RequireAuth` 注解的 JWT 拦截器，按需保护 API 接口安全

### 国际化 i18n 支持
- **中英文切换**：完整的中文（zh.json）和英文（en.json）语言包
- **语言持久化**：自动检测浏览器语言 + localStorage 记住用户偏好
- **React i18next 集成**：使用 `react-i18next` 实现组件级国际化，支持动态切换
- **全局语言选择器**：Layout 底部导航集成语言切换按钮

### 图形验证码
- **登录安全防护**：登录页面增加图形验证码验证，防止暴力破解
- **图片生成**：后端 java.awt 生成干扰线 + 旋转字符的 PNG 图片
- **Redis 存储校验**：验证码文本存入 Redis（5 分钟过期），一次性使用校验
- **点击刷新**：点击验证码图片即刷新，不影响其他表单输入

### 用户体验优化
- **护眼深绿色主题**：将蓝紫色系改为 `#6B8F71` 护眼深绿色，降低长时间使用的视觉疲劳
- **配置 CI/CD 流水线**：GitHub Actions 自动构建 + 测试（编译 → 单元测试 → 前端打包）
...
![国际化中英文切换](images/i18n.png)

护眼深绿色主题全站效果：

![护眼绿色主题](images/green-theme.png)

### 简历与面试

面试中心：

![面试中心](images/interviewcenter.png)
Skill 出题 + JD 解析：

![Skill 出题 + JD 解析](images/JD-explanation.png))

简历库：

![简历库](images/resume-database.png)

简历上传分析：

![简历上传分析](images/resume-upload-analysis.png)

简历分析详情：

![简历分析详情](images/resume-analysis-details.png)
面试记录：

![面试记录](images/interview-record.png)

面试详情：

![面试详情](images/interview-details.png)
...
![面试安排](images/interview-arrange.png)

多模型切换 + 语音服务设置：

![管理聊天模型、向量模型和模块配置](images/AI-model.png)

### 知识库

知识库管理：

![知识库管理](images/knowledge-management.png)
问答助手：

![问答助手](images/question-assistant.png)
...
│   │   │   ├── annotation/           # @RateLimit、@RequireAuth 可重复注解
...
│   │   │   └── security/             # Spring Security 预留（空目录，未实现）
│   │   ├── infrastructure/           # 基础设施
│   │   │   ├── export/               # PDF 导出
...
├── frontend/                         # 前端应用
│   ├── src/
│   │   ├── api/                      # API 接口（含 auth.ts、captcha.ts）
│   │   ├── components/               # 公共组件（含 LanguageSwitcher）
│   │   ├── hooks/                    # 业务 Hooks（含 useLocale）
│   │   ├── i18n/                     # 国际化语言包（二开新增）
│   │   ├── pages/                    # 页面组件（含 LoginPage、RegisterPage）
│   │   ├── types/                    # 类型定义
│   │   └── utils/                    # 工具函数
│   ├── package.json
│   └── vite.config.ts
...
2. 点击"注册"链接，填写用户名和密码完成注册
3. 注册后自动登录，即可使用简历分析、模拟面试等全部功能
4. 下次访问可直接在登录页使用用户名密码登录（需填写图形验证码）
